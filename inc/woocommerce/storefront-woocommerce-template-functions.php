...
if ( ! function_exists( 'storefront_woo_cart_available' ) ) {
	/** Проверяет, доступен ли экземпляр WooCart в запросе
	 * @since 2.6.0
	 * @return bool */
...
if ( ! function_exists( 'storefront_before_content' ) ) {
	/** Перед Содержанием
	 * Оборачивает весь контент WooCommerce в обертки, соответствующие разметке темы
	 * @since   1.0.0
	 * @return  void */
...
if ( ! function_exists( 'storefront_cart_link_fragment' ) ) {
	/** Фрагменты тележки
	 * Убедитесь, что содержимое корзины обновляется при добавлении товаров в корзину с помощью AJAX
...
if ( ! function_exists( 'storefront_cart_link' ) ) {
	/** Ссылка на корзину
	 * Отображается ссылка на корзину с указанием количества товаров и общей суммы в корзине
	 * @return void
	 * @since  1.0.0 */
	function storefront_cart_link() {
		...
		?>
			<a class="cart-contents goodville-cart__link" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'storefront' ); ?>">
				<div class="goodville-cart__wrap">
					<svg><use xlink:href="#cart"></use></svg>
					<span>Cart</span>
				</div>
				<?php /* translators: %d: number of items in cart */ ?>
				<?php /* (отключили отображение) <?php echo wp_kses_post( WC()->cart->get_cart_subtotal() ); ?> <span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'storefront' ), WC()->cart->get_cart_contents_count() ) ); ?></span> */?> 
			</a>
		<?php
	}
}
...
if ( ! function_exists( 'storefront_header_cart' ) ) {
	...
	function storefront_header_cart() {
		if ( storefront_is_woocommerce_activated() ) {
			...
			?>
		<ul id="site-header-cart" class="site-header-cart menu goodville-header__cart">
...
if ( ! function_exists( 'storefront_upsell_display' ) ) {
	/** Распродажи
	 * Замените функцию увеличения продаж по умолчанию на нашу собственную, которая отображает столбцы с правильным количеством товаров
	 * @since   1.0.0
	 * @return  void
...
if ( ! function_exists( 'storefront_sorting_wrapper_close' ) ) {
	/** Сортировочная обертка закрывается
	 * @since   1.4.3
	 * @return  void */
...
if ( ! function_exists( 'storefront_product_columns_wrapper' ) ) {
	/** Упаковка для столбцов продукта
	 * @since   2.2.0
	 * @return  void */
...
if ( ! function_exists( 'storefront_loop_columns' ) ) {
	/** Столбцы цикла по умолчанию в архивах продуктов
...
if ( ! function_exists( 'storefront_product_columns_wrapper_close' ) ) {
	/** Закрытие обертки столбцов продукта
	 * @since   2.2.0
	 * @return  void */
...
if ( ! function_exists( 'storefront_woocommerce_pagination' ) ) {
	/** Разбивка на страницы витрины магазина WooCommerce
...
if ( ! function_exists( 'storefront_product_categories' ) ) {
	/** Отображать Категории Товаров
	 * Подключено к действию "домашняя страница" в шаблоне домашней страницы
	 * @since  1.0.0
	...
	function storefront_product_categories( $args ) {
		...
		/** Отображать раздел только в том случае, если короткий код возвращает категории товаров */
...
if ( ! function_exists( 'storefront_recent_products' ) ) {
	...
	function storefront_recent_products( $args ) {
		...
		/** Отображать раздел только в том случае, если короткий код возвращает товары */
...
if ( ! function_exists( 'storefront_featured_products' ) ) {
	/** Отображать Рекомендуемые Товары
	 * Подключено к действию "домашняя страница" в шаблоне домашней страницы
	 * @since  1.0.0
	 * @param array $args the product section args.
	 * @return void */
...
if ( ! function_exists( 'storefront_best_selling_products' ) ) {
	/** Покажите Самые Продаваемые Продукты
	 * Подключено к действию "домашняя страница" в шаблоне домашней страницы
	 * @since 2.0.0
	 * @param array $args the product section args.
	 * @return void */
...
if ( ! function_exists( 'storefront_promoted_products' ) ) {
	/** Рекомендуемые и поступающие в продажу продукты
...
if ( ! function_exists( 'storefront_handheld_footer_bar' ) ) {
	/** Отображение меню, предназначенного для использования на портативных устройствах
	 * @since 2.0.0 */
...
if ( ! function_exists( 'storefront_single_product_pagination' ) ) {
	/** Навигация на странице продукта к следующему или к предыдущему продукту
	 * @since 2.3.0 */
	function storefront_single_product_pagination() {
		...
		//Показывать только товары в одной категории?
...
if ( ! function_exists( 'storefront_woocommerce_brands_archive' ) ) {
	/** Отображать изображение бренда в архивах брендов
	 * Requires WooCommerce Brands.
	 * @since  2.3.0
	 * @link   https://woocommerce.com/products/brands/
	 * @uses   is_tax()
...
if ( ! function_exists( 'storefront_woocommerce_brands_single' ) ) {
	/** Вывод изображения бренда продукта для использования на отдельных страницах продукта
	 * Requires WooCommerce Brands.
	 * @since  2.3.0
	 * @link   https://woocommerce.com/products/brands/
	 * @uses   storefront_do_shortcode()
