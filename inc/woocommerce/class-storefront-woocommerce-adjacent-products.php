...
if ( ! class_exists( 'Storefront_WooCommerce_Adjacent_Products' ) ) :
	...
	class Storefront_WooCommerce_Adjacent_Products {
		...
		/** Должен ли пост быть в одном и том же термине таксономии.
		 * @var bool */
		private $in_same_term = false;

		/** Список исключенных идентификаторов терминов.
		 * @var string */
		...
		/** Следует ли извлекать предыдущий продукт.
		 * @var bool */
		...
		/** Получите соседний продукт или вернитесь к первому / последнему действительному продукту.
		 * @since 2.4.3
		...
		public function get_product() {
			...
			//Попробуйте получить действительный товар с помощью "get_adjacent_post()`.
			...
			//Действительный товар не найден; Запросите WC для первого / последнего товара.
		...
		/** Запросите в WooCommerce первый или последний товар.
		 * @since 2.4.3
		...
		private function query_wc() {
			...
			//Требуется не менее 2 результатов, в противном случае предыдущий / следующий будут одинаковыми.
